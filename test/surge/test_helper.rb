...
class Surge::Test::SingletonClient < Surge::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Surge::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Surge::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @surge = Surge::Test::SingletonClient.instance
  end
